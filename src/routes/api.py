...
# Pre-Auth Routes
@router.get("/")
async def home_route(db: Session = Depends(get_db)):
    return await home_controller.home(db)

@router.post("/register")
async def register_route(user_data: RegisterLoginSchema, db: Session = Depends(get_db)):
    return await auth_controller.register(user_data, db)

@router.post("/login")
async def login_route(user_data: RegisterLoginSchema, db: Session = Depends(get_db)):
    return await auth_controller.login(user_data, db)

# Auth Routes
@router.get("/me")
async def get_user_profile_route(current_user: dict = Depends(is_authenticated), db: Session = Depends(get_db)):
    return await user_controller.get_user_profile(current_user, db)

@router.put("/me")
async def update_user_profile_route(user_data: UserUpdate, current_user: dict = Depends(is_authenticated), db: Session = Depends(get_db)):
    return await user_controller.update_user_profile(user_data, current_user, db)

@router.delete("/me")
async def delete_user_profile_route(current_user: dict = Depends(is_authenticated), db: Session = Depends(get_db)):
    return await user_controller.delete_user_profile(current_user, db)

# Post Routes
@router.get("/posts/", response_model=List[Post])
async def get_all_posts_route(db: Session = Depends(get_db), skip: int = 0, limit: int = 10, sort: str = "id"):
...
async def create_post_route(post_data: PostCreate, current_user: dict = Depends(is_authenticated), db: Session = Depends(get_db)):
    return await post_controller.create_post(post_data, current_user, db)

@router.put("/posts/{post_id}")
async def update_post_route(post_id: int, post_data: PostUpdate, current_user: dict = Depends(is_authenticated), db: Session = Depends(get_db)):
    return await post_controller.update_post(post_id, post_data, current_user, db)

@router.delete("/posts/{post_id}")
async def delete_post_route(post_id: int, current_user: dict = Depends(is_authenticated), db: Session = Depends(get_db)):
    return await post_controller.delete_post(post_id, current_user, db)

# Comment Routes
@router.get("/posts/{post_id}/comments")
async def get_comments_for_post_route(post_id: int, db: Session = Depends(get_db)):
...
async def delete_comment_route(comment_id: int, current_user: dict = Depends(is_authenticated), db: Session = Depends(get_db)):
